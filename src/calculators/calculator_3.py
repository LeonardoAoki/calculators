...
class Calculator3:
    ...
    def calculate(self, request: FlaskRequest) -> Dict: # type: ignore
        body = request.json
        input_data = self.__validation_body(body)
    ...
    def __validation_body(self, body: Dict) -> List[float]:
    ...
    def __verify_results(self, variance: float, multiplication: float) -> None:
        if variance > multiplication:
            raise HttpBadRequestError("Falha no processo: variância maior que multiplicação")
